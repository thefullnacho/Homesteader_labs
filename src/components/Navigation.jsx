...
const Navigation = ({ setView, cartCount, currentView, isDark, toggleDarkMode }) => (
    <nav className="border-b-2 border-stone-900 bg-[var(--bg-primary)] sticky top-0 z-50 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div className="hidden md:flex gap-8 text-sm items-center">
                ...
                <button 
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleDarkMode(); }}
                    className="p-2 border border-stone-900 hover:bg-stone-900 hover:text-white transition-all"
                    title="TOGGLE_THEME"
                >
            ...
            <div className="flex items-center gap-4">
                ...
                <button 
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleDarkMode(); }}
                    className="md:hidden p-1 border border-stone-900"
                >
                    {isDark ? <Sun size={14} /> : <Moon size={14} />}
                </button>
            </div>
        </div>
        <div className="md:hidden flex justify-around border-t border-stone-900 py-2 bg-[var(--bg-secondary)] text-xs">
            {['SHOP', 'ARCHIVE', 'FABRICATION', 'WEATHER'].map((item) => (
                <button
                    key={item}
                    onClick={() => setView(item)}
